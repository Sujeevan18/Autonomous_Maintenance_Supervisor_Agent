"""
risk_only_baseline.py

Single-Component (Risk-Only) Baseline Model for the
...
produced by the upstream Failure Risk Prediction component (risk_10, risk_30, risk_50),
completely ignoring RUL and Anomaly outputs.

Ablation Value
--------------
Evaluating this model measures the isolated predictive performance of Failure Risk probabilities alone
and highlights the necessity of multi-horizon fusion for autonomous supervisor decisions.

Thresholds:
----------
- risk_10 >= 0.82 -> immediate_maintenance
- risk_30 >= 0.72 -> schedule_maintenance
- risk_30 >= 0.55 -> schedule_inspection
- risk_50 >= 0.40 -> monitor_closely
...
Run from the Backend/ directory:
    ...
    "C:\\Python313\\python.exe" -m app.services.Maintenance_Supervisor.baselines.risk_only_baseline

Expected outputs
----------------
- reports/Maintenance_Supervisor/risk_only_baseline_report.json
...
_REPORT_PATH: Final[Path] = REPORTS_ROOT / "risk_only_baseline_report.json"
...
class RiskOnlyBaselineModel:
    """Risk-Only Single Component Decision Baseline."""
    ...
    def predict_row(self, row: pd.Series) -> str:
        ...
        if r10 >= self.cfg.critical_risk_10_threshold:
            return "immediate_maintenance"
        if r30 >= self.cfg.maintenance_risk_30_threshold:
            return "schedule_maintenance"
        if r30 >= self.cfg.inspection_risk_30_threshold:
            return "schedule_inspection"
        if r50 >= self.cfg.monitoring_risk_50_threshold:
...
def run_risk_only_baseline(input_path: Path | None = None) -> dict:
    ...
    section("RISK-ONLY BASELINE STARTED")
    logger.info("Evaluating dataset: %s", input_path)

    start_time = time.perf_counter()
    df = pd.read_csv(input_path, low_memory=False)

    model = RiskOnlyBaselineModel()
    ...
    report = {
        "status": "success",
        "baseline_name": "risk_only_baseline",
    ...
    section("RISK-ONLY BASELINE COMPLETED")
...
def main() -> int:
    try:
        res = run_risk_only_baseline()
        print(json.dumps(res, indent=2))
        return 0
    except Exception as exc:
        logger.error("Error in risk_only_baseline: %s", exc, exc_info=True)
        return 1
