...
Standalone Trainer for LightGBM Classifier Supervisor Model.

Purpose
-------
Trains, evaluates, and serializes the LightGBM model using preprocessed
...
from app.services.Maintenance_Supervisor.models.lightgbm_supervisor import LightGBMSupervisorModel, HAS_LIGHTGBM
...
_MODEL_OUTPUT_PATH: Final[Path] = ARTIFACT_ROOT / "models" / "lightgbm.joblib"
_REPORT_PATH: Final[Path] = REPORTS_ROOT / "train_lightgbm_report.json"
...
def train_lightgbm(
    ...
    if not HAS_LIGHTGBM:
        logger.warning("LightGBM package is not installed. Skipping train_lightgbm execution.")
    ...
    section("TRAINING LIGHTGBM SUPERVISOR STARTED")
    ...
    section("TRAINING LIGHTGBM SUPERVISOR COMPLETED")
    return report
...
def main() -> int:
    try:
        ...
        logger.error("Error in train_lightgbm: %s", exc, exc_info=True)
        return 1
