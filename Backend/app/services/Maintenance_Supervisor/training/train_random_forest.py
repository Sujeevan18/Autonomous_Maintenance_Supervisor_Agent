...
Standalone Trainer for Random Forest Ensemble Supervisor Model.

Purpose
-------
Trains, evaluates, and serializes the Random Forest model using preprocessed
...
_MODEL_OUTPUT_PATH: Final[Path] = ARTIFACT_ROOT / "models" / "random_forest.joblib"
_REPORT_PATH: Final[Path] = REPORTS_ROOT / "train_random_forest_report.json"
...
def train_random_forest(
    ...
    section("TRAINING RANDOM FOREST SUPERVISOR STARTED")
    ...
    model = RandomForestSupervisorModel()
    ...
    section("TRAINING RANDOM FOREST SUPERVISOR COMPLETED")
    return report
...
def main() -> int:
    try:
        ...
        logger.error("Error in train_random_forest: %s", exc, exc_info=True)
        return 1
